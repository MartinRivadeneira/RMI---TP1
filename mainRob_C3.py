...
class MyRob(CRobLinkAngs):
    ...
    def WriteMap(self, MAP, MAP_y_initial, MAP_x_initial, list_beacons_coords):
        ...
        # Write txt file
        with open('planningmypath.txt', 'w') as f:
            f.write(MAP_str5)
    ...
    def BestPath(self, list_beacons_coords):
        ...
        MAP_y_initial, MAP_x_initial, _ = list_beacons_coords[0]
        list_beacons_coords.pop(0)

        MAP_y_initial_2 = MAP_y_initial
        MAP_x_initial_2 = MAP_x_initial

        beacons_order = [0]

        for abc in range(0, len(list_beacons_coords)+1):
            ...
            print(len_movements)
            print(geral_list_movements)
            idx_next_cell = len_movements.index(min(len_movements))
            print('The closes path is = ' + str(geral_list_movements[idx_next_cell]))

            movement_total.append(geral_list_movements[idx_next_cell])
            MAP_y_initial, MAP_x_initial, beacon = list_beacons_coords[idx_next_cell]
            beacons_order.append(beacon)
            list_beacons_coords.pop(idx_next_cell)

            if len(list_beacons_coords) == 0:
                list_beacons_coords.append((MAP_y_initial_2, MAP_x_initial_2, 0))

        print('List beacons = ' + str(list_beacons_coords))
        print('\n\nThe total path is = ' + str(movement_total))
        print('The order of visited beacons is = ' + str(beacons_order))
    ...
    def BestPathUnknown(self, list_beacons_coords):
        ...
        for abc in range(0, len(list_beacons_coords)+1):
            ...
            for idx in range(0, len(list_beacons_coords)):
                ...
                MAP_for_path = np.array(MAP_copy) * 0
                MAP_for_path[MAP_y_initial, MAP_x_initial] = 1
                # print(MAP_for_path)
            ...
            movement_total_unknown.append(geral_list_movements[idx_next_cell])
            MAP_y_initial, MAP_x_initial, beacon = list_beacons_coords[idx_next_cell]
            beacons_order.append(beacon)
            list_beacons_coords.pop(idx_next_cell)
        ...
        print('List beacons = ' + str(list_beacons_coords))
        print('\n\nThe total path is = ' + str(movement_total_unknown))
        print('The order of visited beacons is = ' + str(beacons_order))

        return movement_total_unknown
